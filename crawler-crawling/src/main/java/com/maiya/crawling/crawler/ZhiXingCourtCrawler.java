 ...
 */
@Component
public class ZhiXingCourtCrawler {

    private static final Logger LOGGER = LoggerFactory.getLogger(ZhiXingCourtCrawler.class);
    ...
    public String startCrawl(RestrictSite site, WebDriver driver, String idCard, String realName) throws Exception {
        ...
        LOGGER.info("开始抓取信息 ZhiXingCourtCrawler idCard {},realName {}", idCard, realName);

        zhiXingReSendCode(site, driver, idCard, realName, 0);
        ...
        requestHeaders.add("Referer", "http://zhixing.court.gov.cn/");
        ...
        for (WebElement aElement : detailElements) {
            ...
            pCodeImgUrl = pCodeImgUrl.substring(0, pCodeImgUrl.lastIndexOf("?")) + "?" + (int) Math.floor(Math.random() * 100);
            ...
            if (StringUtils.isNotEmpty(site.getCodeImgElement())) {//验证码图片
                ...
                try {
                    ...
                    if (aMap.isEmpty()) {
                        LOGGER.info("执行网站解析一行结果为空,responseBody is {} ", response.getBody());
                        continue;
                    }
                } catch (RestClientException e) {
                    LOGGER.error("抓取执行接口出现错误,{}", e);
                }
     ...
     * @return
     */
    private void zhiXingReSendCode(RestrictSite site, WebDriver driver, String idCard, String realName, int reSendNum) throws Exception {
        reSendNum++;
        if (reSendNum > MaiyaCrawlConstants.DEFAULT_CALL_COURT_COUNT) {
            ...
            throw new CustomBusinessException(ReturnCodeEnum.INVALID_PARAMETERS,
                    "身份证" + idCard + ",姓名" + realName + ",执行网站输入" + (reSendNum - 1) + "验证码都失败");
        }
        driver.switchTo().frame(site.getContentFrameElement());
        Thread.sleep(CaptchaConverter.randomInt());
        WebElement errorMessage = driver.findElement(DecideByClass.getBy("/html/body/div/h4/span", "xpath"));
        if (errorMessage == null) {
            LOGGER.info("没有查询到结果的block,名称为resultListBlock");
            return;
        }
        if (errorMessage.getText().contains("验证码错误")) {

            LOGGER.info("ZhiXingCourtCrawler 出现验证码错误,第{}次填写验证码", reSendNum);
            ...
            zhiXingReSendCode(site, driver, idCard, realName, reSendNum);
        }
    }
